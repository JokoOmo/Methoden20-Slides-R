...
```{r, echo=FALSE}
DATACHECK <-fread(here("data/T6/wb_example.csv"))
...
data_al_exp <- fread(here("data/T6/wb_example.csv")) %>%
  rename(Land=iso2c, 
         ...
         Arbeitslosigkeit=SL.UEM.TOTL.ZS) %>%
  select(-country) %>%
  pivot_longer(cols = any_of(c("Exporte","Arbeitslosigkeit")),
               names_to = "Variable", 
               values_to = "Wert") %>%
  pivot_wider(id_cols = any_of(c("Land", "Variable")), 
              names_from = "Jahr", 
              values_from = "Wert") 
data_al_exp
```
...
```{r}
data_gini_bip_right_join <- right_join(data_BIP, data_gini, 
                                       by=c("Jahr"="year", "Land"="country"))
...
Mit Hilfe von `any_of()` können wir Spalten über `character` ansprechen 
(zudem gibt R hier keinen Fehler aus wenn eine Spalte nicht existiert, daher 
auch der Name):

```{r}
head(
  select(data_al_exp_tidy, any_of(c("Land", "Jahr"))), 2)
```

Auch der Ausschluss von Spalten funktioniert mit `any_of()`:

```{r}
head(
  select(data_al_exp_tidy, -any_of(c("Land", "Jahr"))), 
  2)
```
...
unemp_data_wb <- fread(here("data/T6/wb_unemployment.csv"), 
  ...
  select(any_of(c("country", "year", "laborforce_female", 
...
offenheits_daten <- fread(
  ...
  select(
    any_of(c("year", "Land", "trade_total_GDP", "gvnt_cons"))
    )
```
